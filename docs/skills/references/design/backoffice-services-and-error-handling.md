...
- **`ErrorHandler.handle()`** (o prescrito): **8 ficheiros** — `TasksList.tsx`, `TaskFormDrawer.tsx`, `TaskDetailDrawer.tsx`, `hooks/useApiCall.ts`, as três páginas de Construção (`ConstructionStagesPage`, `ConstructionSubStagesPage`, `ConstructionExpensesPage`) e `MyProfileDrawer.tsx` (2026-09-16).
- **`message.error('…')` direto do antd, string PT fixa, sem passar pelo mapa de códigos**: **18 ficheiros** — todo o domínio de empreendimentos (`CreateEnterpriseDrawer`, `EnterpriseViewDrawer`, os cinco `edit/Edit*Card`, `EnterprisesList`), os drawers de construção (`ConstructionExpenseUpsertDrawer`), funcionários (`CreateEmployeeDrawer`, `EmployeeContextMenu`, `EmployeesList`), perfil (`ProfileView`), `InvitesDrawer`, `MapLocationPickerDrawer`, `MediaUploadsSection`, `AcceptInvitePage`, `Login`. `MyProfileDrawer` (ex-`MyProfileModal`) migrou para `ErrorHandler.handle()` + `notificationService` a 2026-09-16, na reescrita para Drawer.
- **Silencioso — só `console.error`, o utilizador não vê nada**: **13 ficheiros**, com destaque para `EnterpriseLocationSection.tsx`, `AuthenticatedImage.tsx` e vários `edit/Edit*Card`.
